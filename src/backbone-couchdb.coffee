  ...
  read_collection : (coll, opts) ->
    ...
    if coll.db?
      ...
      if coll.db.view?
        ...
        startkey = coll.db.startkey ? null
        endkey = coll.db.endkey ? null
        limit = coll.db.limit ? null
        skip = coll.db.skip ? null
        group = coll.db.group ? false
        group_level = coll.db.group_level ? null
        include_docs = coll.db.include_docs ? null
    ...
    if keys then options.keys = keys
    if startkey then options.startkey = startkey
    if endkey then options.endkey = endkey
    if limit then options.limit = limit
    if skip then options.skip = skip
    if group then options.group = group
    if group_level then options.group_level = group_level
    if include_docs then options.include_docs = include_docs
    @helpers.make_db().view "#{@config.ddoc_name}/#{_view}", options
